...
const PIE_COLORS = ["#15157d", "#0057c0", "#4c00b5"];

const TIME_SERIES_DATA = [
  { month: "Jan", hires: 4, applicants: 120 },
  { month: "Feb", hires: 7, applicants: 210 },
  { month: "Mar", hires: 6, applicants: 180 },
  { month: "Apr", hires: 10, applicants: 320 },
  { month: "May", hires: 14, applicants: 450 },
  { month: "Jun", hires: 18, applicants: 520 },
];
  ...
  return (
    <div className="px-6 py-6 pb-12 max-w-7xl mx-auto space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Line Chart */}
        <div className="glass-card rounded-2xl p-5 border border-outline-variant/50">
          <h2 className="font-bold text-on-surface mb-4">Hires Over Time</h2>
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={TIME_SERIES_DATA}>
              <CartesianGrid strokeDasharray="3 3" stroke="#c7c5d420" vertical={false} />
              <XAxis dataKey="month" tick={{ fontSize: 11, fill: "#464652" }} />
              <YAxis yAxisId="left" tick={{ fontSize: 11, fill: "#464652" }} />
              <Tooltip
                contentStyle={{ background: "#fff", border: "1px solid #c7c5d4", borderRadius: 8, fontSize: 12 }}
              />
              <Line yAxisId="left" type="monotone" dataKey="hires" stroke="#15157d" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        {/* Donut Chart */}
        <div className="glass-card rounded-2xl p-5 border border-outline-variant/50 flex flex-col">
          <h2 className="font-bold text-on-surface mb-4">Source Breakdown</h2>
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie
                data={SOURCE_DATA}
          ...
          <div className="flex justify-center gap-4 mt-2">
            {SOURCE_DATA.map((s, i) => (
              <div key={s.name} className="flex items-center gap-1.5 text-xs">
                ...
                <span className="font-medium text-on-surface-variant">{s.name}</span>
              </div>
            ))}
          </div>
        </div>
